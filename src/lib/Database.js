; (function () {
    var _Database = function () {
		...
		sqlexecuteor = function (transaction, sqlObjects, finalSuccess, failure) {
		    ...
		    var sequenceNumber = 0,
				...
				successCallback = function (transaction, results) {
				    ...
				    if (Database.logInfo) {
				        console.log("Success executeing: " + sqlObjects[sequenceNumber].sql);
				    }

				    //execute the results of a select puting them in a much more manageable array form.
				    if (Database.putSelectResultsInArray && isSelectStmt(sqlObjects[sequenceNumber].sql)) {
				...
				failureCallback = function (transaction, error) {
				    if (Database.logErrors) {
				        Database.logErrors("Error: " + error.message + " while executeing statment " + (sequenceNumber + 1) + ": " + sqlObjects[sequenceNumber].sql);
				    }
        ...
        return {
            ...
            openDatabase: function (name, ver, displayname, size, whenOpen) {
                Database.database = window.openDatabase(name, ver, displayname, size);
                readTransactionAvailable = typeof Database.database.readTransaction === 'function';
                if (whenOpen) {
                    whenOpen();
                }
            },

            execute: function (sqlInput, finalSuccessCallback, failureCallback) {
                /*
                *
                *	Arguments:
                *	
                ...
                */
                if (Database.database) {
                    ...
                    if (isUndefined(failureCallback)) {
                        failureCallback = Database.defaultFailureCallback;
                    }

                    if (statementsAreSelectOnly(sqlObjects) && readTransactionAvailable) {
                        Database.database.readTransaction(function (transaction) {
                            sqlexecuteor(transaction, sqlObjects, finalSuccessCallback, failureCallback);
                        });
                    } else {
                        Database.database.transaction(function (transaction) {
                            sqlexecuteor(transaction, sqlObjects, finalSuccessCallback, failureCallback);
                        });
                    }
                } else {
                    ...
                    if (Database.logErrors) {
                        Database.logErrors("Error: Database needs to be opened before sql can be executeed.");
                    }
                    return false;
                }
            },
            ...
            changeVersion: function (oldVersion, newVersion, sqlInput, finalSuccessCallback, failureCallback) {
                /* This is the same as Database.execute but used when you want to change the
                ...
                * executeed and the version of the database is changed to the new version.
                *
                *	Arguments:
                ...
                */
                if (Database.database) {
                    if (Database.database.version === oldVersion) {
                        var sqlObjects = sqlObjectCreator(sqlInput);
                        ...
                        Database.database.changeVersion(oldVersion, newVersion, function (transaction) {
                            sqlexecuteor(transaction, sqlObjects, finalSuccessCallback, failureCallback);
                        });
                    }
                } else {
